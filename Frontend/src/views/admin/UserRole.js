...
import { registerUserRole, getAlluserRole, deleteUserRole } from '../../Api/userRoleApi.js'; // Assuming registerCourse is exported from the API file
...
export default function Group() {
    ...
    if (userRoleData && userRoleData.data) {
        ...
        data = userRoleData.data.map((role) => ({
            _id: role._id, // Include the _id in the data
            "Role": role.role
        }));
    }


    const addButtonName = "New Role";
    const totalBadgeName = "Role";
    ...
    const handleSubmit = async (e) => {
        ...
        setSubmitting(true);
        try {
            const response = await registerUserRole(formData);
                    ...
                    <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                        <div className="mt-10 relative inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl rtl:text-right dark:bg-white-900 sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6">
                            <div>
                                ...
                                <div className="mt-2">
                                    <h3 className="text-lg font-medium leading-6 text-gray-800 capitalize dark:text-black text-center" id="modal-title">Add New Role</h3>
                                    <form onSubmit={handleSubmit} className="mt-4" action="#">
                                        <label className="block mt-3" htmlFor="category">
                                            <input type="text" name="role" id="role" placeholder="Role" value={formData.role} onChange={handleChange} className="block w-full px-4 py-3 text-sm text-gray-700 bg-white border border-gray-200 rounded-md focus:border-blue-400 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-40 dark:border-gray-600 dark:bg-white-900 dark:text-black-300 dark:focus:border-blue-300" />
                                        </label>


                                        <div className="mt-4 sm:flex sm:items-center sm:-mx-2">
                                            <button type="button" onClick={handleCloseModal} className="w-full px-4 py-2 text-sm font-medium tracking-wide text-gray-700 capitalize transition-colors duration-300 transform border border-gray-200 rounded-md sm:w-1/2 sm:mx-2 dark:text-black-200 dark:border-gray-700 focus:outline-none focus:ring focus:ring-gray-300 focus:ring-opacity-40" disabled={submitting}>
                                                Cancel
                                            </button>
